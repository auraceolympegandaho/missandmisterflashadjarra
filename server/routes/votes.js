...
router.post("/init", async (req, res) => {
  try {
    ...
    const pricePerVote = Number(priceRes.rows[0].value);
...
router.get("/status/:id", async (req, res) => {
  try {
    ...
    console.error(err);
    res.status(500).json({ error: "Impossible de verifier le statut." });
  }
});
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (!fedapayTxId) return res.status(200).send("ok");

      const localRes = await pool.query(
        "SELECT * FROM transactions WHERE fedapay_transaction_id = $1",
        [String(fedapayTxId)]
      );
      const localTx = localRes.rows[0];
